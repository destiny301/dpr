...
def main(args):
    ...
    #     l = len(train_loader.dataset.cached_data)
    # for data in test_loader:
    #     l = len(test_loader.dataset.cached_data)
    
    # train_loader.dataset.set_use_cache(cache=True)
    # test_loader.dataset.set_use_cache(cache=True)
    ...
    modelpath = os.path.join(modelfolder, 'swinnet_pyramid_final.pth')
    best_modelpath = os.path.join(modelfolder, 'swinnet_pyramid_best.pth')
    ...
    for epoch in range(args.epoch):
        ...
        # train model
        for xtr, ytr, a128tr, a64tr, a32tr in train_loader:
            ...
            if args.loss == 'wce':
                ...
                ltr2 = criterion(att64, a64tr)
                ltr3 = criterion(att32, a32tr)
        ...
        with torch.no_grad():
            for xte, yte, ate, a64te, a32te, h, w, name in test_loader:
                xte, yte, ate = xte.to(device), yte.to(device), ate.to(device)
                
                pte, _, p32te = model(xte)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--data', type=str, help='which dataset to use(mots/coco/DUTS/MSRA10K/bdd10k/bdd10ks)', default='mots015')
    ...
    argparser.add_argument('--batchsz', type=int, help='batch size(12/15/32/64/128)', default=8)
    argparser.add_argument('--loss', type=str, help='loss function(bce/ce/wce)', default='bce')
    argparser.add_argument('--beta', type=float, help='fn/fp ratio', default=100)
