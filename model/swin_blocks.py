...
class PatchUp(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, x):
        B, L, C = x.shape
        assert L == self.patches_resolution[0]*self.patches_resolution[1], "Don't match with image size"

        if self.norm is not None:
            x = self.norm(x)
        # print(x.shape)
        x = x.transpose(1, 2).view(B, C, self.patches_resolution[0], self.patches_resolution[1]) # B, C, h, w
        # print(x.shape)
        x = self.proj(x)
        # print(x.shape)
        return x
...
class PatchConnect(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # mutual attention
        self.norm1_x_ma = norm_layer(dim)
        self.norm2_xd_ma = norm_layer(dim)
        self.mutualAttn = MutualAttention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.norm3_x_ma = norm_layer(dim)
        self.norm4_xd_ma = norm_layer(dim)
        self.mlp_x_ma = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_xd_ma = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, xd):
        # mutual attention
        x_fuse, xd_fuse = self.drop_path(self.mutualAttn(self.norm1_x_ma(x), self.norm2_xd_ma(xd)))

        x = x + x_fuse
        # depth_fea = depth_fea + depth_fea_fuse

        x = x + self.drop_path(self.mlp_x_ma(self.norm3_x_ma(x)))
